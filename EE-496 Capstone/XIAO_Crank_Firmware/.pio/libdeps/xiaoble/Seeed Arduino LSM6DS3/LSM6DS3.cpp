...
#if defined(ARDUINO_XIAO_RA4M1) || defined(ARDUINO_ARCH_RP2040) ||             \
...
status_t LSM6DS3Core::writeRegister(uint8_t offset, uint8_t dataToWrite) {
  ...
  case SPI_MODE:
    ...
    // No way to check error on this write (Except to read back but that's not
    // reliable)
...
float LSM6DS3::readTempC(void) {
  float output = (float)readRawTemp() /
...
uint32_t LSM6DS3::fifoTimestamp() {
  uint8_t data[6];
  status_t error = readRegisterRegion(data, LSM6DS3_ACC_GYRO_FIFO_DATA_OUT_L,
                                      3 * sizeof(uint16_t));
...
void LSM6DS3::fifoBegin(void) {
  ...
  if (settings.timestampFifoEnabled == 1) {
    ...
    fifo_ctrl2 |=
        LSM6DS3_ACC_GYRO_TIM_PEDO_FIFO_EN_ENABLED; // FIFO_CTRL2(07H)->TIMER_PEDO_FIFO_EN
  ...
  // Select the decimation factor for the 4th FIFO dataset by configuring the
  // DEC_DS4_FIFO[2:0] field in the FIFO_CTRL4 register.
